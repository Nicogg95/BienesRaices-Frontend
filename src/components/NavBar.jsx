...
const NavBar = ({clase}) => {
  ...
  return (
    <nav className={`${clase} md:flex flex-col items-center gap-y-7 md:flex-row md:opacity-1`}>
        ...
        {agente?._id ? 
        (<Link className="hover:text-yellow-500 hover:font-bold text-xl" onClick={()=>cerrarSesion()}>Cerrar Sesion</Link>)
        :
        (<Link className="hover:text-yellow-500 hover:font-bold text-xl" to="/login">Iniciar Sesion</Link>)}
    </nav>
  )
}
